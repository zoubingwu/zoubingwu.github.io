...
习惯了回调函数的思路在刚接触到 promise 的时候很难去改变，虽然强迫了自己使用 promise，但在第一层 then 函数内部又开始继续使用回调函数或者嵌套的 then 函数调用，这样的代码非常的不 promisey，在阅读时也和之前的回调地狱一样，完全没有达到使用 promise 的目的。我们完全可以改成下面这样的写法：
